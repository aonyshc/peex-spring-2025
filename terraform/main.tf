...
// Create an internet gateway named "demo_igw"
// and attach it to the "demo_vpc" VPC
...
resource "aws_subnet" "demo_public_subnet" {
  ...
  // Put the subnet into the "demo_vpc" VPC
  vpc_id            = aws_vpc.demo_vpc.id
  ...
  // We are tagging the subnet with a name of "demo_public_subnet_" and
  // suffixed with the count
  tags = {
    Name = "demo_public_subnet_${count.index}"
  }
}
...
resource "aws_subnet" "demo_private_subnet" {
  ...
  // We are tagging the subnet with a name of "demo_private_subnet_" and
  // suffixed with the count
  tags = {
    Name = "demo_private_subnet_${count.index}"
  }
}

// Create a public route table named "demo_public_rt"
resource "aws_route_table" "demo_public_rt" {
  // Put the route table in the "demo_vpc" VPC
  vpc_id = aws_vpc.demo_vpc.id
...
resource "aws_route_table_association" "public" {
  ...
  // "demo_public_rt" from above
  route_table_id = aws_route_table.demo_public_rt.id
  
  // This is the subnet ID. Since the "demo_public_subnet" is a 
...
// Create a private route table named "demo_private_rt"
...
// Create a security group for the RDS instances called "demo_db_sg"
resource "aws_security_group" "demo_db_sg" {
  // Basic details like the name and description of the SG
  name        = "demo_db_sg"
  description = "Security group for demo databases"
  // We want the SG to be in the "demo_vpc" VPC
  vpc_id      = aws_vpc.demo_vpc.id
...
// Create a db subnet group named "demo_db_subnet_group"
resource "aws_db_subnet_group" "demo_db_subnet_group" {
  ...
  // loop through our private subnets in "demo_private_subnet" and
  // add them to this db subnet group
  subnet_ids  = [for subnet in aws_subnet.demo_private_subnet : subnet.id]
}

// Create a DB instance called "demo_database"
resource "aws_db_instance" "demo_database" {
  ...
  // settings.database.db_name variable, which is set to "demo"
  ...
  // This is the DB subnet group "demo_db_subnet_group"
...
// Create an EC2 instance named "demo_web"
resource "aws_instance" "demo_web" {
  ...
  // The subnet ID for the EC2 instance. Since "demo_public_subnet" is a list
  ...
  // "demo_public_subnet" and putting the EC2 instance in there
  subnet_id              = aws_subnet.demo_public_subnet[count.index].id
  ...
  // We are tagging the EC2 instance with the name "demo_db_" followed by
  // the count index
  tags = {
    Name = "demo_web_${count.index}"
  }
}

// Create an Elastic IP named "demo_web_eip" for each
// EC2 instance
resource "aws_eip" "demo_web_eip" {
  ...
  count    = var.settings.web_app.count
	...
	// The EC2 instance. Since demo_web is a list of 
  ...
  instance = aws_instance.demo_web[count.index].id

	// We want the Elastic IP to be in the VPC
  vpc      = true

	// Here we are tagging the Elastic IP with the name
	// "demo_web_eip_" followed by the count index
  tags = {
    Name = "demo_web_eip_${count.index}"
  }
}
